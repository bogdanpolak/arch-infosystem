...
__fastcall TBrowserDlg::TBrowserDlg(TComponent* Owner) : TForm(Owner) {
}
// ---------------------------------------------------------------------------
...
bool __fastcall TBrowserDlg::isServiced() {
	...
	String client_kind(DataSource1->DataSet->FieldByName("RODZAJ_OSB")
		->AsString);

	if (client_kind != "F") {
		serviced = false;

		ShowMessage(_T("Nieobs³ugiwany rodzaj klienta: '") + client_kind +
			_T("'"));
	}
