...
__fastcall TPersonDlg::TPersonDlg(TComponent* Owner)
   : TForm(Owner)
{
}
//---------------------------------------------------------------------------
...
{
   String typ_dok_toz(PersonModule->cdsPerson->FieldByName("TYP_DOK_TOZ")->AsString);
...
void __fastcall TPersonDlg::dsAddressDataChange(TObject *Sender, TField *Field)
{
   if(Field == NULL)
   {
      String typ_adr(PersonModule->cdsAddress->FieldByName("TYP_ADRESU")->AsString);
...
{
   if(Application->MessageBox(_T("Czy na pewno anulowaæ wprowadzone zmiany?"), _T("Zapisywanie danych klienta"), MB_YESNO) == IDYES)
   {
...
{
   if(Application->MessageBox(_T("Czy zapisaæ wprowadzone zmiany?"), _T("Zapisywanie danych klienta"), MB_YESNO) == IDYES)
   {
      PersonModule->Post();
...
void __fastcall TPersonDlg::Info(int person_id)
{
   try
   {
      ...
      dnaNavigator->VisibleButtons = dnaNavigator->VisibleButtons >> nbInsert >> nbDelete >> nbEdit >> nbPost >> nbCancel;
...
{
   if(Application->MessageBox(_T("Czy na pewno usun¹æ dane wybranego klienta?"), _T("Usuwanie danych klienta"), MB_YESNO | MB_DEFBUTTON2) != IDYES)
   {
      return false;
   }
