...
bool __fastcall TDatabaseModule::Login(const String& user, const String& passw)
{
   ...
   if(!UserData->IsEmpty())
   {
      ...
      m_user.Status   = klienci::UserInfo::stringToUserStatus(UserData->FieldByName("STATUS")->AsString);
      ...
      {
         m_user.Rights.push_back(UserRights->FieldByName("KOD_FUNSYS")->AsString);
