...
const _TCHAR* const cc_PersonQuMainUpdOrIns =
_T("update or insert into bo_main(id_osoby, rodzaj_osb, nazwa, nip, kod_kraju, regon, email, telefon, rb_numer, rej_data, rej_oper, mod_data, mod_oper)\
...
const _TCHAR* const cc_PersonQuDetailUpdOrIns =
_T("update or insert into bo_fiz(id_osoby, imie, imie_drugie, nazwisko, imie_ojca, imie_matki, pesel, typ_dok_toz, nr_dok_toz, data_ur, miejsce_ur)\
...
void __fastcall TPersonModule::cdsAddressAfterInsert(TDataSet *DataSet)
{
   ...
   DataSet->FieldByName("REJ_OPER")->AsInteger  = DatabaseModule->GetUser().Ident;
}
//---------------------------------------------------------------------------
...
void __fastcall TPersonModule::cdsAddressBeforePost(TDataSet *DataSet)
{
   ...
   DataSet->FieldByName("MOD_OPER")->AsInteger  = DatabaseModule->GetUser().Ident;
}
//---------------------------------------------------------------------------

void __fastcall TPersonModule::dspPersonBeforeApplyUpdates(TObject *Sender, OleVariant &OwnerData)
{
   m_updateError = "";
   ...
   if(!cdsPerson->IsEmpty())
   {
      ...
      if(person_id == -1)
      {
         ...
         cdsPerson->FieldByName("REJ_OPER")->AsInteger  = DatabaseModule->GetUser().Ident;
      }
      else
      {
         cdsPerson->FieldByName("MOD_DATA")->AsDateTime = Now();
         cdsPerson->FieldByName("MOD_OPER")->AsInteger  = DatabaseModule->GetUser().Ident;
      }

      cdsPerson->FieldByName("NAZWA")->AsString = cdsPerson->FieldByName("NAZWISKO")->AsString + " " + cdsPerson->FieldByName("IMIE")->AsString;
...
void __fastcall TPersonModule::dspPersonUpdateError(TObject *Sender, TCustomClientDataSet *DataSet, EUpdateError *E,
...
void __fastcall TPersonModule::dspPersonBeforeUpdateRecord(TObject *Sender, TDataSet *SourceDS, TCustomClientDataSet *DeltaDS,
   TUpdateKind UpdateKind, bool &Applied)
{
   ...
   else if(SourceDS == quAddress)
   {
      if(UpdateKind == ukInsert)
      {
         ...
         DeltaDS->FieldByName("ID_ADRESU")->AsInteger = spAddressID->ParamByName("ID_ADRESU")->AsInteger;

         DeltaDS->FieldByName("REJ_DATA")->AsDateTime = Now();
         DeltaDS->FieldByName("REJ_OPER")->AsInteger  = DatabaseModule->GetUser().Ident;
         ...
         {
            DeltaDS->FieldByName("ID_OSOBY")->AsInteger = cdsPerson->FieldByName("ID_OSOBY")->AsInteger;
         }
