...
void __fastcall TSeekClientDlg::Edit1KeyPress(TObject *Sender,
	System::WideChar &Key) {
	if (Key == '\r') {
		Key = 0;
...
void __fastcall TSeekClientDlg::Button1Click(TObject *Sender) {
	...
	if (client_id > 0) {
		...
		DatabaseModule->SelectData->ParamByName("ID_OSOBY")->AsInteger =
			client_id;
	}
	else {
		...
		DatabaseModule->SelectData->ParamByName("NAZWA")->AsString =
			Edit1->Text + (EndsText(_T("%"), Edit1->Text) ? _T("") : _T("%"));
	}
	...
	if (!DatabaseModule->SelectData->IsEmpty()) {
		...
		if (found) {
			m_clientID = DatabaseModule->SelectData->FieldByName("ID_OSOBY")
				->AsInteger;

			m_clientKind = DatabaseModule->SelectData->FieldByName("RODZAJ_OSB")
				->AsString;
